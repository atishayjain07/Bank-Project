...
public class Transfer implements ActionListener,ItemListener
{
Frame f;
Button deposit,profile,transfer,withdraw,passbook,b6,logout;
Label bankname,subheading,heading;

Label name,source,destination,amount,rupee,detail,cusname;
Button transferb;
Choice sourcec,destinationc;
TextField amountt;
TextArea detailta;
Button home;
Connection co;
...
heading=new Label("        Transfer Amount");
...
destination=new Label("Select Destination Account No. : ");
amount=new Label("Amount : ");
rupee=new Label("Rs");
cusname=new Label("");
detail=new Label("Details : ");
transferb=new Button("Transfer Amount");
sourcec=new Choice();
destinationc=new Choice();
...
transferb.setForeground(Color.green);
...
name.setBounds(465,250,170,30);
cusname.setBounds(640,250,180,30);
source.setBounds(430,290,210,30);
sourcec.setBounds(635,290,200,40);
destination.setBounds(400,330,240,30);
destinationc.setBounds(635,330,200,40);
...
transferb.setBounds(640,510,180,40);
...
destinationc.addItemListener(this);
...
pst=co.prepareStatement("select name,acno,amount from customer");
rs=pst.executeQuery();
while(rs.next())
{
if(rs.getInt(2)==Integer.parseInt(Login.usernamet.getText()))
{
continue;
}
else
{
//destinationc.add(rs.getString(1)+" | acc_no :  "+Integer.toString(rs.getInt(2)));
destinationc.add(Integer.toString(rs.getInt(2)));
}
}
rs.close();
pst.close();
...
{
String x=destinationc.getSelectedItem();
//x.charAt(x.length());
System.out.println(x);
desacno=Integer.parseInt(x);
}
...
if(amt<0)
{
System.out.println("sorry you have no money to transfer");
}
else
{
...
pst=co.prepareStatement("UPDATE customer SET amount=amount+? WHERE acno=?");
pst.setInt(1,Integer.parseInt(amountt.getText()));
pst.setInt(2,desacno);
System.out.println(Integer.parseInt(amountt.getText()));
pst.executeUpdate();
}
